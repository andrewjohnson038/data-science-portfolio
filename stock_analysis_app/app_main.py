...
import yfinance as yf  # Import yfinance (used to retrieve financial data from yahoo finance)
...
import numpy as np  # Import NumPy for data wrangling / calculations
...
forecasted_year_range_slider = st.sidebar.slider("Choose a Forecast Range (Years):", 1, 10)  # creates a slider for forecast years. 1 and 10 are the year range
...
selected_stock_peg_ratio = selected_stock_metrics_df['PEG Ratio'].values[0]  # float or str (if pulled from Alpha Vantage)
...
selected_stock_net_profit_margin = selected_stock_metrics_df['Net Profit Margin'].values[0]  # float (as decimal) or None
selected_stock_yoy_revenue_growth = selected_stock_metrics_df['YOY Revenue Growth'].values[0]  # float (as decimal) or None
selected_stock_yoy_ocfg_growth = selected_stock_metrics_df['YOY Operating Cash Flow Growth'].values[0]  # float (as decimal) or None
...
selected_stock_fifty_two_week_range = selected_stock_metrics_df['52-Week Range'].values[0]  # str (e.g., "88.00 - 180.00")
...
selected_stock_earnings_date = selected_stock_metrics_df['Earnings Date'].values[0]  # list or pandas.Timestamp (yfinance often returns a list of earnings dates)
...
selected_stock_hist_daily_VaR_95, selected_stock_hist_daily_VaR_95_dollars = AppData.calculate_historical_VaR(selected_stock_price_history_df,
                                                                                time_window='daily')
selected_stock_hist_monthly_VaR_95, selected_stock_hist_monthly_VaR_95_dollars = AppData.calculate_historical_VaR(selected_stock_price_history_df,
                                                                                    time_window='monthly')
selected_stock_hist_yearly_VaR_95, selected_stock_hist_yearly_VaR_95_dollars = AppData.calculate_historical_VaR(selected_stock_price_history_df,
                                                                                  time_window='yearly')
...
selected_stock_trained_model, selected_stock_forecasted_df = data.get_forecasted_data_df(selected_stock_price_history_df, selected_stock_forecasted_year_range)
...
selected_stock_score, selected_stock_grade, selected_stock_grade_color_background, selected_stock_grade_color_outline, selected_stock_score_details = model.calculate_grades(
    selected_stock, selected_stock_metrics_df, selected_stock_forecasted_df, industry_avg_df, selected_stock_mc_sim_df, selected_stock_sharpe_ratio,
...
with kpi_col4:
    ...
    with kpi_col4:
        ...
        chosen_forecasted_price = selected_stock_forecasted_df['yhat'].iloc[-1]  # forecast['yhat'].iloc[-1] retrieves the forecasted value
...
home_tab1, home_tab2, home_tab3, home_tab4 = st.tabs(
    ["Historical/Current Data", "Forecasted Data", "Stock Grades", "Chat-Bot"])
    ...
    with sh_c:
                ...
                if not selected_stock_price_history_df.empty:
                    ...
                    if selected_stock_regular_market_price is not None:
                        ...
                        sh_col1_1.metric(label="Current Price:", value="$" + str(
                            round(selected_stock_regular_market_price, 2)))  # round # to 2 decimal points and make a string
                    else:
                        # Write the data is not available for the field if missing
                    ...
                    if selected_stock_regular_market_open is not None:
                        ...
                        sh_col1_1.metric(label="Open Price:", value="$" + str(
                            round(selected_stock_regular_market_open, 2)))  # round # to 2 decimal points and make a string
                    else:
                        # Write the data is not available for the field if missing
                    ...
                    if selected_stock_regular_market_volume is not None:
                        selected_stock_regular_market_volume = "{:,}".format(round(selected_stock_regular_market_volume, 2))  # adds commas
                        sh_col1_2.metric(label=f"Today's Trade Volume:", value=str(selected_stock_regular_market_volume))
                    ...
                    # If variable from info is missing, check if we can get from history table
                        ...
                        today_vol = selected_stock_price_history_df.loc[selected_stock_price_history_df['Date'] == dv.today.strftime('%Y-%m-%d'), 'Volume']
                    ...
                    if selected_stock_debt_to_equity is not None:
                        # Write the value to the app for today in KPI format if the data is available
                        sh_col1_1.metric(label="Debt-to-Equity:", value=str(round(selected_stock_debt_to_equity * 100, 2)) + '%')
                    else:
                        # Write the data is not available for the field if missing
                    ...
                    if selected_stock_dividend_yield is not None:
                        # Write the value to the app for today in KPI format if the data is available
                        sh_col1_1.metric(label="Dividend Yield:", value=str(round(selected_stock_dividend_yield, 2)) + "%")
                    else:
                        # Write the data is not available for the field if missing
                    ...
                    if selected_stock_net_profit_margin is not None:
                        # Write the value to the app for today in KPI format if the data is available
                        sh_col1_1.metric(label="Net Profit Margin:", value=str(round(selected_stock_net_profit_margin * 100, 2)) + "%")
                    else:
                        # Write the data is not available for the field if missing
                    ...
                    if selected_stock_price_to_sales is not None:
                        # Write the value to the app for today in KPI format if the data is available
                        sh_col1_2.metric(label="Price-to-Sales:", value=str(round(selected_stock_price_to_sales, 2)))
                    else:
                    ...
                    if selected_stock_dividend_rate is not None:
                        # Write the value to the app for today in KPI format if the data is available
                        sh_col1_2.metric(label="Dividend Rate (Annual):", value='$' + str(round(selected_stock_dividend_rate, 2)))
                    else:
                        # Write the data is not available for the field if missing
                    ...
                    if selected_stock_fifty_two_week_range is not None:
                        # Write the value to the app for today in KPI format if the data is available
                        sh_col1.metric(label=f"52 Week Range:", value="$" + str(selected_stock_fifty_two_week_range))
                    else:
                        # Write the data is not available for the field if missing
        ...
        with sh_col2:
            ...
            with st.container(border=True):
                ...
                smoothed_prices = data.apply_exponential_smoothing(selected_stock_price_history_df, smoothing_level=0.002)  # can change alpha
        ...
        with st.expander(f"{selected_stock_company_name} Overview - {selected_stock_sector} / {selected_stock_industry}"):
        ...
        with sh_c.container(border=True):
            ...
            with sh_col1.container(border=True):
                def plot_mov_avg_data():  # define a function for our plotted data (fig stands for figure)
                    ...
                    one_yr_moving_average_data = selected_stock_moving_average_data[selected_stock_moving_average_data['Date'] >= dv.one_yr_ago]
        ...
        with sh_c.container(border=True):
            ...
            with risk_col1:
                st.metric(label="1-Day VaR at 95% Confidence", value=f"{selected_stock_hist_daily_VaR_95 * 100:.2f}%",
                          delta=f"${selected_stock_hist_daily_VaR_95_dollars:.2f}", delta_color="inverse")

            with risk_col2:
                st.metric(label="1-Month VaR at 95% Confidence", value=f"{selected_stock_hist_monthly_VaR_95 * 100:.2f}%",
                          delta=f"${selected_stock_hist_monthly_VaR_95_dollars:.2f}", delta_color="inverse")

            with risk_col3:
                st.metric(label="1-Year VaR at 95% Confidence", value=f"{selected_stock_hist_yearly_VaR_95 * 100:.2f}%",
...
with home_tab2:
    ...
    with fs_c:
        ...
        with fs_graph_c:
            ...
            fig1 = plot_plotly(selected_stock_trained_model, selected_stock_forecasted_df)  # plot visual (plotly needs the model and forecast to plot)
        ...
        with fs_price_metric:
            ...
            with fs_price_metric1:
                ...
                fs_price_metric1.metric(f"Forecast Year: {chosen_forecasted_year}",
                                        f"${str(round(chosen_forecasted_price, 2))}", f"{trend_difference_percentage}%")
            ...
            with fs_price_metric2:
                ...
                yoy_avg_fr_price_change = round((int(chosen_forecasted_price) - int(selected_stock_regular_market_price)) / (
                        int(chosen_forecasted_year) - int(dv.current_yr)), 2)

                # Get the Average Change %
                yoy_avg_fr_price_change_pct = round((yoy_avg_fr_price_change / selected_stock_regular_market_price) * 100, 0)
        ...
        with fs_components_c:
            ...
            fig2 = selected_stock_trained_model.plot_components(selected_stock_forecasted_df)  # write component visuals to label fig2.
